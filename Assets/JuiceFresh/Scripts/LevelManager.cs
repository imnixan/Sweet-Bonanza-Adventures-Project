...
{
    [SerializeField]
    private UnityEngine.UI.Text coinsReward;
    ...
    public BoostIcon ActivatedBoost
    {
        ...
        set
        {
            ...
            {
                if (
                    activatedBoost.type == BoostType.ExtraMoves
                    || activatedBoost.type == BoostType.ExtraTime
                )
                {
    ...
    public GameState gameStatus
    {
        ...
        set
        {
            ...
            else if (value == GameState.Win)
            {
                passLevelCounter++;
                OnMenuComplete();
                int reward = Mathf.RoundToInt(Score / 1500);
                PlayerPrefs.SetInt("Gems", PlayerPrefs.GetInt("Gems") + reward);
                PlayerPrefs.Save();
                InitScript.Gems = PlayerPrefs.GetInt("Gems");
                coinsReward.text = reward.ToString();
                GameObject
                    .Find("CanvasGlobal")
                    .transform.Find("MenuComplete")
                    .gameObject.SetActive(true);
                OnWin();
            }
        }
    ...
    void Scale(float defaultScale) //1.4.9
    {
        ...
        float width =
            GetSquare(maxCols - 1, 0).transform.position.x - GetSquare(0, 0).transform.position.x;
        ...
        else
        {
            ...
            if (aspect == 2.17f) //1.4.9
                panel.anchoredPosition =
                    new Vector2(panel.anchoredPosition.x, 935) + Vector2.down * 100;
        }
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            itemExplPool[i] =
                Instantiate(
                    Resources.Load("Prefabs/Effects/ItemExpl"),
                    transform.position,
                    Quaternion.identity
                ) as GameObject;
        ...
        {
            flowersPool[i] =
                Instantiate(flower, transform.position, Quaternion.identity) as GameObject;
    ...
    void InitTargets()
    {
        ...
        if (targetBlocks > 0 && target == Target.BLOCKS)
        {
            ...
            blocksObject.GetComponent<TargetGUI>().text.GetComponent<Counter_>().totalCount =
                targetBlocks;
            //CreateCollectableTarget(ingrObject, target, false);
        }
        else if (LevelManager.THIS.target == Target.CAGES)
        {
            ...
            cageTargetObject.GetComponent<TargetGUI>().text.GetComponent<Counter_>().totalCount =
                TargetCages;
            //CreateCollectableTarget(ingrObject, target, false);
        }
        else if (LevelManager.THIS.target == Target.BOMBS)
        {
            ...
            bombTargetObject.GetComponent<TargetGUI>().text.GetComponent<Counter_>().totalCount =
                bombsCollect;
            //CreateCollectableTarget(ingrObject, target, false);
        }
    ...
    public void CreateCollectableTarget(
    ...
    )
    {
        ...
        int Sprites_Length = (Resources.Load("Prefabs/Item") as GameObject)
            .GetComponent<Item>()
            .items.Length;
        ...
        for (int i = 0; i < num; i++)
        {
            if (collectionItems[i] != (object)0 && ingrTarget[i].count > 0)
            {
                ...
                ingr.transform.Find("CountIngr").GetComponent<Counter_>().totalCount = ingrTarget[
                    i
                ].count;
    ...
    IEnumerator InitBombs()
    {
        ...
        List<Item> itemsRand = GetRandomItems(
            bombsCollect - bombsOnField - LevelManager.THIS.TargetBombs
        ); //1.3
        int i = 0;
                ...
                if (_item.GetComponent<Item>() != null)
                {
                    if (_item.GetComponent<Item>().currentType == ItemsTypes.NONE)
                    {
                        if (_item.GetComponent<Item>().color == (int)collectItems[i] - 1)
                        {
                            ...
                            spr.sprite = _item.GetComponent<Item>().items[
                                _item.GetComponent<Item>().color
                            ];
    ...
    IEnumerator StartAnimateIngredient(GameObject item, int i)
    {
        ...
        AnimationCurve curveX = new AnimationCurve(
            new Keyframe(0, item.transform.localPosition.x),
            new Keyframe(0.4f, ingr[i].transform.position.x)
        );
        AnimationCurve curveY = new AnimationCurve(
            new Keyframe(0, item.transform.localPosition.y),
        ...
        while (distCovered < 0.5f)
        {
            ...
            item.transform.localPosition = new Vector3(
                curveX.Evaluate(distCovered),
                curveY.Evaluate(distCovered),
                0
            );
    ...
    public void CheckWinLose()
    {
        if (Limit <= 0)
        {
            ...
            else if (LevelManager.THIS.target == Target.CAGES && LevelManager.THIS.TargetCages > 0)
            {
                ...
                LevelManager.THIS.target == Target.COLLECT
                || LevelManager.THIS.target == Target.ITEMS
            )
            {
                ...
                LevelManager.THIS.target == Target.SCORE
                && LevelManager.Score < GetScoresOfTargetStars()
            )
            {
                lose = true;
            }
            if (
                LevelManager.Score < LevelManager.THIS.star1
                && LevelManager.THIS.target != Target.SCORE
            )
            {
                lose = true;
    ...
    IEnumerator PreWinAnimationsCor()
    {
        ...
        GameObject
            .Find("Level/Canvas")
            .transform.Find("PreCompleteBanner")
            .gameObject.SetActive(true); //1.4.5
        yield return new WaitForSeconds(3);
        GameObject
            .Find("Level/Canvas")
            .transform.Find("PreCompleteBanner")
            .gameObject.SetActive(false); //1.4.5
                ...
                if (hit != null)
                {
                    ...
                    if (item.currentType != ItemsTypes.INGREDIENT)
                    {
                        ...
                        { //boost action events   BOMB
                            ...
                            LevelManager.THIS.ActivatedBoost.type == BoostType.Shovel
                            && item.currentType != ItemsTypes.INGREDIENT
                        )
                            ...
                            )
                            {
                                if (destroyAnyway.Count > 1)
                                {
                                    Vector2 pos1 = new Vector2(
                                        destroyAnyway[destroyAnyway.Count - 1].square.col,
                                ...
                                { //add item to selection
                                    ...
                                    int selectingSoundNum = Mathf.Clamp(
                                        destroyAnyway.Count - 1,
                                        0,
                                        9
                                    );
                                    SoundBase.Instance.PlaySound(
                                        SoundBase.Instance.selecting[selectingSoundNum]
                                    );
                                    if (
                                        (
                                            destroyAnyway.Count
                                                % (extraItemEvery + extraCageAddItem)
                                            == 0
                                        )
                                ...
                                { //remove item from selection (step back by finger)
                                    ...
                                    if (destroyAnyway.Count > 1)
                                    {
                                        ...
                                        {
                                            if (
                                                destroyAnyway[destroyAnyway.Count - 1].currentType
                                                == ItemsTypes.HORIZONTAL_STRIPPED
                                            )
                                                gatheredTypes.Remove(
                                                    gatheredTypes[gatheredTypes.Count - 1]
                                                );
                                            else if (
                                                destroyAnyway[destroyAnyway.Count - 1].currentType
                                            ...
                                            destroyAnyway[
                                                destroyAnyway.Count - 1
                                            ].square.SetActiveCage(false);
                                            HighlightManager.DeselectItem(
                                                destroyAnyway[destroyAnyway.Count - 1],
                                                item
                                            );
                                            destroyAnyway.Remove(
                                                destroyAnyway[destroyAnyway.Count - 1]
                                            );
                    ...
                    { //1.3
                        ...
                        if (!isIngredient)
                        {
                            if (LevelManager.THIS.ActivatedBoost.type == BoostType.Bomb)
                            {
                                ...
                                GameObject obj =
                                    Instantiate(
                                        Resources.Load("Prefabs/Effects/bomb"),
                            ...
                            else if (LevelManager.THIS.ActivatedBoost.type == BoostType.Shovel)
                            {
                                ...
                                GameObject obj =
                                    Instantiate(
                                        Resources.Load("Prefabs/Effects/shovel"),
                                        square.transform.position,
                                        square.transform.rotation
                                    ) as GameObject;
                                obj.GetComponent<SpriteRenderer>().sortingOrder = 5;
                            ...
                            else if (LevelManager.THIS.ActivatedBoost.type == BoostType.Energy)
                            {
                                ...
                                GameObject obj =
                                    Instantiate(
                                        Resources.Load("Prefabs/Effects/energy"),
                                        square.transform.position,
                                        square.transform.rotation
                                    ) as GameObject;
                                obj.GetComponent<SpriteRenderer>().sortingOrder = 5;
    ...
    void AnimateField(Vector3 pos)
    {
        ...
        AnimationCurve curveX = new AnimationCurve(
            new Keyframe(0, pos.x + 15),
            new Keyframe(0.7f, pos.x - 0.2f),
            new Keyframe(0.8f, pos.x)
        );
        AnimationCurve curveY = new AnimationCurve(
            new Keyframe(0, pos.y + yOffset),
            ...
            if (row == 0 || col == 0 || col == maxCols - 1 || row == maxRows - 1)
            {
                ...
                { //top left
                    ...
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.left * 0.01f + Vector3.up * 0.01f;
                }
                ...
                { //top right
                    ...
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.right * 0.01f + Vector3.up * 0.01f;
                }
                ...
                { //bottom left
                    ...
                    outline.transform.localRotation = Quaternion.Euler(0, 0, -90);
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.left * 0.01f + Vector3.down * 0.01f;
                }
                ...
                { //bottom right
                    ...
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.right * 0.01f + Vector3.down * 0.01f;
                }
            }
            else
            {
                //top left
                if (
                    GetSquare(col - 1, row - 1, true).type == SquareTypes.NONE
                ...
                )
                {
                    ...
                    spr.sprite = outline3;
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.left * 0.015f + Vector3.up * 0.015f;
                    outline.transform.localRotation = Quaternion.Euler(0, 0, 180);
                }
                //top right
                if (
                ...
                )
                {
                    ...
                    spr.sprite = outline3;
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.right * 0.015f + Vector3.up * 0.015f;
                    outline.transform.localRotation = Quaternion.Euler(0, 0, 90);
                }
                //bottom left
                if (
                ...
                )
                {
                    ...
                    spr.sprite = outline3;
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.left * 0.015f + Vector3.down * 0.015f;
                ...
                )
                {
                    ...
                    spr.sprite = outline3;
                    outline.transform.localPosition =
                        Vector3.zero + Vector3.right * 0.015f + Vector3.down * 0.015f;
                    outline.transform.localRotation = Quaternion.Euler(0, 0, 0);
                }
            }
        }
    ...
    void CreateObstacles(int col, int row, GameObject square, SquareTypes type)
    {
        ...
        )
        {
            ...
            (
                levelSquaresFile[row * maxCols + col].obstacle == SquareTypes.DOUBLESOLIDBLOCK
                && type == SquareTypes.NONE
            )
                ...
                if (GetSquare(col, row) != null)
                {
                    ...
                    )
                    {
                        if (
                            (
                                GetSquare(col, row).item == null
                                && !GetSquare(col, row).IsHaveSolidAbove()
                            ) || !falling
                        )
                        {
    ...
    { //activate boosts from map
        ...
        if (BoostColorfullBomb > 0)
        {
            ...
            GameObject colorMix =
                Instantiate(Resources.Load("Prefabs/Effects/colorful_mix")) as GameObject;
    ...
    public List<Item> GetIngredients(int i = -1)
    {
        ...
        foreach (GameObject item in items)
        {
            ...
            {
                if (
                    item.GetComponent<Item>().currentType == ItemsTypes.INGREDIENT
    ...
    public List<List<Item>> GetMatches(
        FindSeparating separating = FindSeparating.NONE,
        int matches = 3
    )
    {
        ...
        for (int col = 0; col < maxCols; col++)
        {
            for (int row = 0; row < maxRows; row++)
            {
                if (GetSquare(col, row) != null)
                {
                    ...
                    {
                        List<Item> newCombine = GetSquare(col, row)
                            .FindMatchesAround(separating, matches, countedSquares);
        ...
        while (true)
        {
            ...
            foreach (List<Item> desrtoyItems in combinedItems)
            {
                if (lastDraggedItem == null)
                {
                    ...
                    {
                        if (lastDraggedItem == null)
                            lastDraggedItem = desrtoyItems[
                                UnityEngine.Random.Range(0, desrtoyItems.Count)
                            ];
                    ...
                    if (item.square.IsCageGoingToBroke())
                    {
                        ...
                        if (iCounter % extraItemEvery == 0)
                        {
                            ...
                            foreach (Item item1 in items)
                            {
                                ...
                                flowerParticle
                                    .GetComponent<Flower>()
                                    .StartFly(item.transform.position);
                                cc++;
                            }
                        }
    ...
    public void CheckHighlightExtraItem(Item item)
    {
        ...
        {
            if (
                _item.currentType == ItemsTypes.HORIZONTAL_STRIPPED
    ...
    {
        GameObject effect =
            Instantiate(stripesEffect, obj.transform.position, Quaternion.identity) as GameObject;
        if (!horrizontal)
    ...
    public void PopupScore(int value, Vector3 pos, int color)
    {
        ...
        if (showPopupScores)
        {
            ...
            if (color <= scoresColors.Length - 1)
            {
                ...
                poptxt.transform.GetComponentInChildren<Outline>().effectColor =
                    scoresColorsOutline[color];
            }
    ...
    void UpdateBar()
    {
        ProgressBarScript.Instance.UpdateDisplay(
            (float)Score * 100f / ((float)star1 / ((star1 * 100f / star3)) * 100f) / 100f
        );
    }
    ...
    void ProcessGameDataFromString(string mapText)
    {
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("COLLECT COUNT "))
            {
                ...
                {
                    if (
                        InitScript.Instance.collectedIngredients.Count <= i
            ...
            else if (line.StartsWith("COLLECT ITEMS "))
            {
                ...
                for (int i = 0; i < blocksNumbers.Length; i++)
                {
                    if (target == Target.COLLECT)
                    {
                        ...
                        {
                            CollectedIngredients ingFromList = InitScript
                                .Instance
            ...
            { //Maps
                ...
                {
                    levelSquaresFile[mapLine * maxCols + i].block = (SquareTypes)
                        int.Parse(st[i][0].ToString());
                    levelSquaresFile[mapLine * maxCols + i].obstacle = (SquareTypes)
