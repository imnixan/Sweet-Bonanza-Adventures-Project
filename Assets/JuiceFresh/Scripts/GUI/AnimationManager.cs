...
public class AnimationManager : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (name == "MenuPlay")
        {
            ...
            LevelManager.THIS.CreateCollectableTarget(
                transform.Find("Image/TargetIngr/TargetIngr").gameObject,
                target
            );
            // for (int i = 1; i <= 3; i++)
            // {
    ...
    public void OnFinished()
    {
        ...
        if (name == "MenuFailed")
        {
            ...
            {
                if (
                    LevelManager.THIS.ingrTarget[0].count > 0
                    || LevelManager.THIS.ingrTarget[1].count > 0
                )
                {
                    TargetCheck(false, 1);
                }
    ...
    IEnumerator FireworkParticles()
    {
        GameObject firework =
            Instantiate(
                fireworkPrefab,
                UnityEngine.Random.insideUnitCircle * 10,
                Quaternion.identity
            ) as GameObject;
    ...
    public void CloseMenu()
    {
        ...
        if (gameObject.name == "MenuComplete")
        {
            ...
            else
            {
                GameObject g =
                    Instantiate(
                        Resources.Load("Prefabs/Congratulations"),
                        Vector2.zero,
                        Quaternion.identity
                    ) as GameObject;
    ...
    public void Buy(GameObject pack)
    {
        ...
        if (pack.name == "Pack1")
        {
            InitScript.waitedPurchaseGems = int.Parse(
                pack.transform.Find("Count").GetComponent<Text>().text.Replace("x ", "")
            );
        }
    ...
    public void BuyLife(GameObject button)
    {
        ...
        {
            InitScript.Instance.SpendGems(
                int.Parse(button.transform.Find("Price").GetComponent<Text>().text)
            );
    ...
    public void MusicOff(GameObject Off)
    {
        ...
        PlayerPrefs.SetInt(
            "Music",
            (int)GameObject.Find("Music").GetComponent<AudioSource>().volume
        );
        PlayerPrefs.Save();
    }
